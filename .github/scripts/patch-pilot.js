...
function installPackage(packageName, version) {
  ...
  run(`npm install ${packageName}@${version} --save-exact`, {
    stdio: "inherit",
  });
}
...
function createPR({ branchName, title, body }) {
  run(`gh pr create --title "${title}" --body "${body}" --head ${branchName}`, {
    stdio: "inherit",
  });
}
...
function findDependencyPaths(tree, target, currentPath = [], results = []) {
  if (!tree) {
    return results;
  }
...
function findNearestSafeVersion({ currentVersion, vulnerableRange, versions }) {
  ...
  return versions
    ...
    .sort(semver.compare)
    .find((version) => {
      const isVulnerable = semver.satisfies(version, vulnerableRange);

      const sameMajor = semver.major(version) === currentMajor;

      const greater = semver.gt(version, currentVersion);

      return !isVulnerable && sameMajor && greater;
    });
}
...
function updateDependencyVersion(packageName, version) {
...
function addOverride(packageName, version) {
  const packageJson = readPackageJson();
...
async function remediate() {
  for (const [packageName, vuln] of Object.entries(vulnerabilities)) {
    const branchName = `patchPilot/${packageName}-${Date.now()}`;

    const dependencyType = vuln.isDirect ? "DIRECT" : "TRANSITIVE";

    const vulnerableRange = vuln.range;

    const fixedVersion = vuln.fixAvailable;

    if (!fixedVersion) {
      log(`Skipping ${packageName} (no fix available)`);
    ...
    let remediationStrategy = "UNKNOWN";
    ...
    if (dependencyType === "DIRECT") {
      ...
      const safeVersion = findNearestSafeVersion({
        currentVersion,
        vulnerableRange,
        versions,
      });
      ...
      if (!safeVersion) {
        log(`No safe version found for ${packageName}`);

        continue;
      }

      remediationStrategy = "DIRECT_UPGRADE";
     ...
     */
      ...
      const paths = findDependencyPaths(dependencyTree, packageName);

      const topParent = paths?.[0]?.[1];

      if (!topParent) {
        log(`No parent found for ${packageName}`);

        continue;
      }

      log(`Top parent for ${packageName}: ${topParent}`);

      const parentVersion = getInstalledVersion(topParent);
      ...
      const safeParentVersion = parentVersions
        .filter((v) => semver.valid(v))
        .sort(semver.compare)
        .find((v) => semver.major(v) === semver.major(parentVersion));
       ...
       */
      if (!fixed) {
        remediationStrategy = "OVERRIDE";

        log(`Applying override for ${packageName}`);

        addOverride(packageName, fixedVersion);
    ...
    const changedFiles = getChangedFilesCount();

    if (changedFiles > 15) {
      log(`Too many changed files (${changedFiles}), skipping`);
    ...
    const title = `fix(security): remediate ${packageName} vulnerability`;
...
`;
    ...
    commitChanges(`fix: remediate ${packageName} vulnerability`);
    ...
    log(`PR created for ${packageName}`);
  }
